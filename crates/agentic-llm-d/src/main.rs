...
struct Cli {
    /// Matches `agentic-server`'s default. Restrict reachability at the network
    /// layer: the endpoints are not yet authenticated.
    ...
    /// Seals the context passed between hydrate and persist. Required: without
...
/// bypass the graceful shutdown and cut off in-flight requests.
#[cfg(unix)]
